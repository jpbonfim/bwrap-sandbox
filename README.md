...
* **D-Bus Host Escape Neutralization & Keyring Scoping:** Passing raw host D-Bus sockets into a sandbox allows attackers to instruct `systemd --user` to run commands outside all namespaces. Profiles requiring D-Bus (e.g. `antigravity`) spawn an isolated `xdg-dbus-proxy` filtering out all calls except `org.freedesktop.secrets` and `org.gnome.keyring` (used for OAuth tokens), while completely blocking `org.freedesktop.systemd1` and other host APIs. *(Note: Enabling `filtered_dbus = true` allows processes inside the sandbox to query unlocked keyring credentials, so only enable it on profiles that strictly require host keyring integration).*
* **Git Hook & Config Trap Neutralization:** Agents cannot escalate privileges or achieve post-sandbox host code execution by dropping malicious triggers into `.git/hooks` or poisoning `.git/config` with `core.fsmonitor`, `core.hooksPath`, `core.pager`, or `diff.external`. The sandbox enforces `--ro-bind` on both `.git/hooks` and `.git/config`, and exports `GIT_CONFIG_PARAMETERS='core.hooksPath=/dev/null'`.
